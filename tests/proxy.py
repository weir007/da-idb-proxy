...
class KeycloakWrapperTester:
    ...
    def test_tenant_management(self):
        ...
        if res.status_code == 201:
            ...
            if admin_user:
                print(f"   [INFO] 租户管理员用户名: {admin_user}")
    ...
    # --- 2' 补充测试 role-by-id 方法
    def test_role_uuid_stability(self):
        print("\n=== [场景 3: 角色 UUID 稳定性测试] ===")
        # 定义初始名字和更新后的名字
        role_name = "role_init_name"
        updated_name = "role_new_name"

        # 1. 创建初始角色 (使用 POST /roles)
        payload = {
            "name": role_name,
            "description": "用于验证 UUID 稳定性的角色"
        }
        res = self.session.post(f"{self.base_url}/{TEST_REALM}/roles", json=payload)
        self.log("1. 创建初始角色", res)

        # 2. 获取该角色的 UUID
        # 因为创建接口不回传 ID，必须先通过名字 GET 一次
        res = self.session.get(f"{self.base_url}/{TEST_REALM}/roles/{role_name}")
        self.log("2. 获取角色详情以拿到 UUID", res)
        role_data = res.json()
        role_id = role_data["id"]
        print(f"   [INFO] 获取到角色 UUID: {role_id}")

        # 3. 通过 UUID 修改角色名称 (使用 PUT /by-id/{id})
        # 注意：Keycloak 要求 PUT 角色时，payload 建议包含 id
        update_payload = {
            "id": role_id,
            "name": updated_name,
        ...
        # 断言逻辑
        assert role_after["id"] == role_id, f"FAILED: UUID 发生了变更！原ID: {role_id}, 新ID: {role_after['id']}"
        assert role_after["name"] == updated_name, f"FAILED: 角色名未更新为 {updated_name}"
        ...
        if res.status_code == 404:
            ...
            print(f"   [WARNING] 角色可能未被删除，状态码: {res.status_code}")
    ...
    def test_idp_management(self):
        ...
        # 记录实际使用的 Alias (后端返回的)
        alias = res.json().get("alias") if res.status_code == 201 else "da-saml-idp"
        ...
        if res_get.status_code == 200:
            ...
            target = next((i for i in instances if i['alias'] == alias), None)
        ...
        del_res = self.session.delete(f"{self.base_url}/{TEST_REALM}/idp/saml/instances/{alias}")
        self.log("清理删除 SAML 实例", del_res)

        print("\n=== [场景 3.1: IDP Mappers 管理] ===")

        res = self.session.post(f"{self.base_url}/{TEST_REALM}/idp/saml/instances", json=idp_payload)
        self.log("创建 SAML 实例", res)

        if res.status_code not in [200, 201]:
            print(f"   [SKIP] 无法创建IDP实例，跳过mapper测试")
            return

        # 2. 创建一个 Mapper (严格匹配 IdPMapperCreate Schema)
        # 注意：identityProviderMapper 是固定的 Provider ID
        mapper_payload = {
            "name": "test-attribute-mapper",
        ...
        res = self.session.post(f"{self.base_url}/{TEST_REALM}/idp/saml/instances/{alias}/mappers", json=mapper_payload)
        self.log("创建 IDP Mapper", res)

        mapper_id = None
        if res.status_code in [200, 201]:
            # 如果后端返回了创建后的对象，则提取 ID；否则从列表里抓第一个
            data = res.json()
            mapper_id = data.get("id")
            print(f"   [INFO] 创建的Mapper ID: {mapper_id}")

        # 3. 获取 mappers 列表
        res = self.session.get(f"{self.base_url}/{TEST_REALM}/idp/saml/instances/{alias}/mappers")
        self.log("获取 IDP Mappers 列表", res)

        if res.status_code == 200:
            mappers = res.json()
            print(f"   [INFO] 当前Mappers数量: {len(mappers)}")
            if not mapper_id and mappers:
                mapper_id = mappers[0].get("id")

        # 4. 更新 Mapper (严格匹配 IdPMapperUpdate Schema)
        if mapper_id:
            update_payload = {
                "name": "updated-attribute-mapper",
                "config": {
                    "user.attribute": "email",
                    "friendly.name": "Email Address",
                    "attribute.name": "emailAddress",
                    "syncMode": "FORCE"  # 变更同步模式
                }
            }
            res = self.session.put(f"{self.base_url}/{TEST_REALM}/idp/saml/instances/{alias}/mappers/{mapper_id}",
                                   json=update_payload)
            # 注意：PUT 成功返回 204，没有 Body
            self.log("更新 IDP Mapper", res)

        # 5. 删除 Mapper
        if mapper_id:
            res = self.session.delete(f"{self.base_url}/{TEST_REALM}/idp/saml/instances/{alias}/mappers/{mapper_id}")
            # 注意：DELETE 成功返回 204
            self.log("删除 IDP Mapper", res)
    ...
    def test_token_exchange(self):
        print("\n=== [场景 7: Token 交换接口] ===")
        print("   [INFO] 注意：此测试需要真实的OIDC授权码，此处仅验证接口可访问性")
        
        # 由于token交换需要真实的OIDC授权码，这里仅验证接口存在
        # 实际测试需要先完成OIDC登录流程获取code
        print("   [INFO] Token交换接口路径: /{realm}/token/exchange")
        print("   [SKIP] 跳过完整测试（需要OIDC登录流程）")
